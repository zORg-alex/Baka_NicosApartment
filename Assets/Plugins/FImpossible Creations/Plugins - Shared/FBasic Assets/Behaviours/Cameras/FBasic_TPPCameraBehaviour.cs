...
namespace FIMSpace.Basics
{
    ...
    public class FBasic_TPPCameraBehaviour : MonoBehaviour {
		...
		private void OnEnable() {
            ...
            mousePos.Enable();
            mouseScroll = new InputAction("M scrooll", InputActionType.Value, "<Mouse>/scroll");
            mouseScroll.Enable();
            tab = new InputAction("Tab", InputActionType.Button, "<Keyboard>/tab", interactions: "press(behavior=1)");
            tab.Enable();
            tab.performed += e => SwitchCalculations();
        }
        ...
        {
            targetDistance -= (mouseScroll.ReadVector2().y * 5f);

            if (!rotateCamera) return;

            targetSphericRotation = mousePos.ReadVector2() * RotationSensitivity;
        }
